...
public final class ClickHouseCast implements ClickHouseExpression {
    ...
    public static ClickHouseConstant castToInt(ClickHouseConstant cons) {
        ...
        case String:
             ...
             * unprintAbleCharThatLetsBecomeNumberZero(asString)) { return
             * ClickHouseCreateConstant.createInt32Constant(0); } for (int i = asString.length(); i >= 0; i--) { try {
             * String substring = asString.substring(0, i); Pattern p = Pattern.compile("[+-]?\\d\\d*"); if
             * (p.matcher(substring).matches()) { BigDecimal bg = new BigDecimal(substring); long result; try { result =
             * bg.longValueExact(); } catch (ArithmeticException e) { if (substring.startsWith("-")) { result =
             * Long.MIN_VALUE; } else { result = Long.MAX_VALUE; } } return
             * ClickHouseCreateConstant.createInt32Constant(result); } } catch (Exception e) {
             *
    ...
    private static ClickHouseConstant convertInternal(ClickHouseConstant value, boolean convertRealToInt,
        ...
        case String:
             ...
             * while (startsWithWhitespace(asString)) { asString = asString.substring(1); } if (!asString.isEmpty() &&
             * unprintAbleCharThatLetsBecomeNumberZero(asString)) { return
             * ClickHouseCreateConstant.createInt32Constant(0); } if (asString.toLowerCase().startsWith("-infinity") ||
             * asString.toLowerCase().startsWith("infinity") || asString.startsWith("NaN")) { return
             * ClickHouseCreateConstant.createInt32Constant(0); } for (int i = asString.length(); i >= 0; i--) { try {
             * String substring = asString.substring(0, i); double d = Double.parseDouble(substring); BigDecimal first =
             * new BigDecimal(substring); long longValue = first.longValue(); BigDecimal second =
             * BigDecimal.valueOf(longValue); boolean isWithinConvertibleRange = longValue >=
             * MIN_INT_FOR_WHICH_CONVERSION_TO_INT_IS_TRIED && longValue <= MAX_INT_FOR_WHICH_CONVERSION_TO_INT_IS_TRIED
             * && convertRealToInt; boolean isFloatingPointNumber = substring.contains(".") ||
             * substring.toUpperCase().contains("E"); boolean doubleShouldBeConvertedToInt = isFloatingPointNumber &&
             * first.compareTo(second) == 0 && isWithinConvertibleRange; boolean isInteger = !isFloatingPointNumber &&
             * first.compareTo(second) == 0; if (doubleShouldBeConvertedToInt || isInteger && !convertIntToReal) { //
             * see https://www.sqlite.org/src/tktview/afdc5a29dc return
             * ClickHouseCreateConstant.createInt32Constant(first.longValue()); } else { return
             * ClickHouseCreateConstant.createFloat64Constant(d); } } catch (Exception e) { } } if (noNumIsRealZero) {
             * return ClickHouseCreateConstant.createFloat64Constant(0.0); } else { return
             * ClickHouseCreateConstant.createInt32Constant(0); }
             *
             */
